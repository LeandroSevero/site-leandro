...
const SUSPICIOUS_PATTERN = /<(script|iframe|img|svg|object|embed|link|style)[\s>\/]/i;
const SUSPICIOUS_ATTRS = /\b(onerror|onclick|onload|onmouseover|onfocus|onblur|onchange|onsubmit)\s*=/i;
const SUSPICIOUS_PROTO = /(javascript|data|vbscript)\s*:/i;

const isString = (v) => typeof v === 'string';

const hasSuspiciousContent = (value) => {
  return (
    SUSPICIOUS_PATTERN.test(value) ||
    SUSPICIOUS_ATTRS.test(value) ||
    SUSPICIOUS_PROTO.test(value)
  );
};
...
const validateTypes = ({ turnstileToken, name, email, phone, subject, message }) => {
  const fields = { turnstileToken, name, email, subject, message };
  for (const [key, val] of Object.entries(fields)) {
    if (!isString(val)) {
      return { field: key === 'turnstileToken' ? null : key, message: 'Formato inválido.' };
    }
  }
  if (phone !== undefined && phone !== null && phone !== '' && !isString(phone)) {
    return { field: 'phone', message: 'Formato inválido.' };
  }
  return null;
};
...
const validateFields = ({ name, email, phone, subject, message }) => {
  ...
  else if (hasSuspiciousContent(n)) errors.name = 'Conteúdo não permitido.';
  ...
  else if (hasSuspiciousContent(e)) errors.email = 'Conteúdo não permitido.';

  const p = isString(phone) ? phone.trim() : '';
  if (p) {
    const digits = p.replace(/\D/g, '');
    if (digits.length < 10 || digits.length > 11) errors.phone = 'Informe um telefone válido: (99) 99999-9999.';
    else if (hasSuspiciousContent(p)) errors.phone = 'Conteúdo não permitido.';
  }
  ...
  else if (hasSuspiciousContent(s)) errors.subject = 'Conteúdo não permitido.';
  ...
  else if (hasSuspiciousContent(m)) errors.message = 'Use apenas texto válido.';
...
const checkRateLimit = async (rateLimits, type, key) => {
  const day = getToday();
  const doc = await rateLimits.findOne({ type: { $eq: type }, key: { $eq: key }, day: { $eq: day } });
...
const incrementRateLimit = async (rateLimits, type, key) => {
  ...
  await rateLimits.updateOne(
    { type: { $eq: type }, key: { $eq: key }, day: { $eq: day } },
    {
      $inc: { count: 1 },
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        <tr>
          <td style="background:#1a5fa8;border-radius:12px 12px 0 0;padding:28px 32px;">
            <p style="margin:0;font-size:11px;font-weight:600;color:rgba(255,255,255,0.6);letter-spacing:0.08em;text-transform:uppercase;">Confirmação de recebimento</p>
            <h1 style="margin:8px 0 0;font-size:20px;font-weight:700;color:#ffffff;">Recebi sua mensagem</h1>
          </td>
        </tr>
        <tr>
          <td style="background:#ffffff;padding:32px;border-radius:0 0 12px 12px;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <p style="margin:0 0 16px;font-size:15px;color:#1e293b;">Olá, ${eName}.</p>
            <p style="margin:0 0 16px;font-size:14px;color:#475569;line-height:1.7;">Obrigado por entrar em contato através do meu site.</p>
            <p style="margin:0 0 24px;font-size:14px;color:#475569;line-height:1.7;">Recebi sua mensagem e responderei assim que possível.</p>
            <p style="margin:0;font-size:14px;color:#475569;line-height:1.7;">Atenciosamente,<br><strong style="color:#1e293b;">Leandro Severo</strong></p>
          </td>
        </tr>
        <tr>
          <td style="padding:20px 0 0;text-align:center;">
            <p style="margin:0;font-size:12px;color:#94a3b8;">Esta é uma resposta automática. Por favor, não responda este e-mail.</p>
          </td>
        </tr>
      </table>
    </td></tr>
  ...
  const row = (label, value) =>
    value
      ? `<tr>
          ...
          <td style="padding:10px 16px;font-size:14px;color:#1e293b;word-break:break-word;">${value}</td>
        </tr>`
      : '';
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        <tr>
          <td style="background:#1a5fa8;border-radius:12px 12px 0 0;padding:28px 32px;">
            ...
            <h1 style="margin:8px 0 0;font-size:20px;font-weight:700;color:#ffffff;">${eSubject}</h1>
          </td>
        </tr>
        <tr>
          <td style="background:#ffffff;padding:0;border-radius:0 0 12px 12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
              ...
              ${row('E-mail', `<a href="mailto:${eEmail}" style="color:#1a5fa8;text-decoration:none;">${eEmail}</a>`)}
              <tr><td colspan="2" style="height:1px;background:#f1f5f9;padding:0;"></td></tr>
              ${ePhone ? row('Telefone', ePhone) + '<tr><td colspan="2" style="height:1px;background:#f1f5f9;padding:0;"></td></tr>' : ''}
              ${row('Mensagem', eMessage)}
              <tr><td colspan="2" style="height:1px;background:#f1f5f9;padding:0;"></td></tr>
...
export default async function handler(req, res) {
  ...
  const body = req.body || {};
  const { turnstileToken, name, email, phone, subject, message } = body;

  const typeError = validateTypes({ turnstileToken, name, email, phone, subject, message });
  if (typeError) {
    if (!typeError.field) {
  ...
  const fieldErrors = validateFields({ name, email, phone: phone || '', subject, message });
  if (fieldErrors) {
  ...
  const cleanPhone = isString(phone) ? phone.trim() : '';
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from: `${escapeHtml(cleanName)} <${fromEmail}>`,
      to: [toEmail],
      reply_to: cleanEmail,
  ...
  try {
    ...
    await resend.emails.send({
      from: `Leandro Severo <${fromEmail}>`,
      to: [cleanEmail],
      subject: 'Recebi sua mensagem',
      html: buildAutoReplyHtml({ name: cleanName }),
    });
  } catch {
  }
