...
const setFormState = (form, submitBtn, state, { message = '' } = {}) => {
  const overlay = form.querySelector('.contact-form-state-overlay');
  ...
  panel.className = 'contact-form-state-overlay';
  ...
  if (state === FORM_STATE.SUCCESS) {
    ...
    panel.innerHTML = `
      <div class="contact-form-result contact-form-result--rate-limit" role="alert" aria-live="assertive">
        ...
        <p class="contact-form-result-text">Você já atingiu o limite de <strong>2 mensagens hoje</strong>. Tente novamente amanhã.</p>
        <button type="button" class="contact-form-result-btn contact-form-result-btn--secondary" data-action="close">
          Fechar
        </button>
      </div>
    `;
...
const handleSubmit = async (form, submitBtn) => {
  ...
  if (!turnstileToken) {
    ...
    submitBtnEl && form.insertBefore(el, submitBtnEl);
    return;
  }
...
const resetFormFully = (form, submitBtn) => {
  ...
  const formBody = form.querySelector('.contact-form-body');
  if (formBody) formBody.removeAttribute('aria-hidden');
...
export const initContactModal = () => {
  ...
  const closeModal = () => {
    ...
    const stateOverlay = form.querySelector('.contact-form-state-overlay');
    if (stateOverlay) {
      stateOverlay.remove();
      const formBody = form.querySelector('.contact-form-body');
      if (formBody) formBody.removeAttribute('aria-hidden');
